...
//MongoClient mongoClient = MongoClients.create("mongodb+srv://akhil:1234@cluster0-6irpd.mongodb.net/test?retryWrites=true");
 mongoose.connect("mongodb+srv://krishnaveni:krishnaveni33@cluster0-cuvh1.mongodb.net/test?retryWrites=true&w=majority",{ useNewUrlParser: true });
	...
	app.get("/blogs",isLoggedIn, function(req, res){
		Blog.find({}, function(err, blogs){
			if(err){
				...
				res.render("index", {blogs: blogs , currentUser: req.user.username});
			}
		});
	});
//NEW ROUTE
app.get("/blogs/new",isLoggedIn,function(req, res){
	res.render("new",  { currentUser: req.user.username });
});
app.get("/blogs/userprof",isLoggedIn,function(req, res){
		res.render("userprof",{ currentUser: req.user.username });
});
...
app.get("/blogs/:id",isLoggedIn,function(req, res){
	Blog.findById(req.params.id, function(err, foundBlog){
		if(err){
			...
			res.render("show", {blog: foundBlog, currentUser: req.user.username});
		}
	});
});
//EDIT ROUTE
app.get("/blogs/:id/edit",isLoggedIn,function(req, res){
	Blog.findById(req.params.id, function(err, foundBlog){
		if(err){
			...
			res.render("edit", {blog: foundBlog ,  currentUser: req.user.username});
		}
	});
});
